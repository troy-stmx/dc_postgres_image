...
setup_database() {
  ...
  su - "${PG_SYS_USER}" -c "${PG_BIN}/psql -h ${PG_HOST} -p ${PG_PORT} -d ${PG_DB} -c 'DO \$do\$ BEGIN ALTER TEXT SEARCH CONFIGURATION chinese_zh ADD MAPPING FOR n,v,a,i,e,l WITH simple; EXCEPTION WHEN others THEN NULL; END \$do\$'"
